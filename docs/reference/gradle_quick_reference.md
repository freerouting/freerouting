...
├── libs/             # All JAR files
│   ├── freerouting-<version>.jar              # Standard JAR
│   ├── freerouting-<version>-executable.jar   # Fat JAR (runnable)
│   ├── freerouting-<version>-javadoc.jar      # API docs (only when publishing)
│   └── freerouting-<version>-sources.jar      # Source code (only when publishing)
...
```powershell
java -jar build/libs/freerouting-<version>-executable.jar
```
