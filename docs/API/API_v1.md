...
Freerouting registers a custom URL protocol handler on all supported platforms (Windows, Linux, macOS). This allows browsers and external applications to launch Freerouting directly via a URL.

### Format

```
freerouting://open?--param1=value1&--param2=value2
```

- The scheme is `freerouting://`
- The host is `open` (required but not interpreted — reserved for future use)
- Each query parameter is a standard `--key=value` CLI argument, separated by `&`
- Values containing special characters (`://`, spaces, `&`, `=`) must be [URL-encoded](https://en.wikipedia.org/wiki/Percent-encoding)

### Examples

**Launch Freerouting in headless API mode:**

```
freerouting://open?--gui.enabled=false&--api_server.enabled=true
```

**Launch with custom router settings:**

```
freerouting://open?--gui.enabled=false&--api_server.enabled=true&--router.max_passes=200&--router.via_costs=42
```

**Launch with a specific endpoint (note URL encoding of `://` and `:`):**

```
freerouting://open?--api_server.enabled=true&--api_server.endpoints=http%3A%2F%2F127.0.0.1%3A37864
```

### Supported Parameters

All `--key=value` command-line arguments documented in [settings.md](../settings.md) are supported. Common parameters for URL-based launching:

| Parameter | Example | Description |
|-----------|---------|-------------|
| `--gui.enabled` | `false` | Disable GUI (headless mode) |
| `--api_server.enabled` | `true` | Enable the API server |
| `--api_server.http_allowed` | `true` | Allow HTTP connections |
| `--api_server.endpoints` | `http://127.0.0.1:37864` | Server listen endpoint (URL-encode the value) |
| `--api_server.cors_origins` | `*` | CORS allowed origins |
| `--api_server.authentication.enabled` | `false` | Disable API authentication (for local EDA plugin use) |
| `--api_server.idle_timeout` | `3600` | Auto-shutdown after N seconds of inactivity (0 = disabled) |
| `--router.max_passes` | `200` | Maximum routing passes |
| `--router.max_threads` | `4` | Maximum routing threads |
| `--router.via_costs` | `42` | Via cost factor |
| `--logging.console.level` | `DEBUG` | Console log level |
| `--logging.file.enabled` | `false` | Disable file logging |

### Platform Registration

| Platform | Mechanism | Scope |
|----------|-----------|-------|
| **Windows** | Registry key `HKCU\Software\Classes\freerouting` (auto-registered on first launch) | Per-user, no admin required |
| **Linux** | `.desktop` file in `~/.local/share/applications/` + `xdg-mime` (auto-registered on first launch) | Per-user |
| **macOS** | `CFBundleURLTypes` in `Info.plist` (registered at build time in the `.app` bundle) | Per-app |

Registration is automatic and idempotent — it runs on every application startup (Windows/Linux) and updates the path if the installation location changes.