...
public class WindowRouteParameter extends BoardSavableSubWindow
{
    ...
    public WindowRouteParameter(BoardFrame p_board_frame)
    {
        ...
        if (this.board_handling.get_routing_board().get_test_level() != app.freerouting.board.TestLevel.RELEASE_VERSION)
        {
    ...
    private class SnapAngle90Listener implements java.awt.event.ActionListener
    {
        ...
        {
            if (board_handling.get_routing_board().rules.get_trace_angle_restriction() == app.freerouting.board.AngleRestriction.NINETY_DEGREE)
            {
                return;
            }
            Collection<app.freerouting.board.Trace> trace_list = board_handling.get_routing_board().get_traces();
    ...
    private class SnapAngle45Listener implements java.awt.event.ActionListener
    {
        public void actionPerformed(java.awt.event.ActionEvent p_evt)
        {
            if (board_handling.get_routing_board().rules.get_trace_angle_restriction() == app.freerouting.board.AngleRestriction.FORTYFIVE_DEGREE)
            {
                return;
            }
            Collection<app.freerouting.board.Trace> trace_list = board_handling.get_routing_board().get_traces();
            ...
            for (app.freerouting.board.Trace curr_trace : trace_list)
            {
                if (curr_trace instanceof app.freerouting.board.PolylineTrace)
                {
                    if (!((app.freerouting.board.PolylineTrace)curr_trace).polyline().is_multiple_of_45_degree())
                    {
                        free_angle_traces_found = true;
                        break;
                    }
