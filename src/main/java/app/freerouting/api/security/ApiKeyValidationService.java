...
/**
 * Singleton service that orchestrates API key validation across one or more configured {@link
 * ApiKeyProvider} implementations.
 *
 ...
 * {@link ApiKeyValidationResult#PROVIDER_FAILED}, access is denied.
 *
 * <h2>Disabled mode</h2>
 *
...
public final class ApiKeyValidationService {
      ...
      for (String providerName : providerNames) {
        ...
        if ("GoogleSheets".equalsIgnoreCase(providerName)) {
          if (authSettings.googleSheets != null
            ...
            FRLogger.warn(
                "GoogleSheets provider configured but sheetUrl or googleApiKey is missing.");
          }
        }
      }
    }
   ...
   *
   * <p>Initialization reads {@link app.freerouting.Freerouting#globalSettings} to determine whether
   * authentication is enabled and which providers to instantiate. Subsequent calls return the
   ...
   *
   * <p>Package-private visibility is intentional — only test classes in the same package should
   * inspect the provider list directly.
   */
   ...
   *
   * @return {@code true} if authentication is required, {@code false} if all requests are allowed
   *     through.
   */
   ...
   * @return {@code true} if validation is disabled or the key is explicitly granted. {@code false}
