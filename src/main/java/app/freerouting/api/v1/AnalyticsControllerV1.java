 ...
 *
 * <ul>
 *   <li>{@code POST /v1/analytics/track} — record a single analytics event (e.g. {@code "API
 *       Endpoint Called"}). Requires the BigQuery service-account key to be configured via {@code
 *       FREEROUTING__USAGE_AND_DIAGNOSTIC_DATA__BIGQUERY_SERVICE_ACCOUNT_KEY}.
 *   <li>{@code POST /v1/analytics/identify} — associate user traits with a user ID or anonymous ID
 *       and persist them to BigQuery ({@code identify} table).
 * </ul>
 *
 * <p>Both endpoints are excluded from API-key validation and environment-host validation (public
 * access), so that remote Freerouting instances can send analytics without needing a caller API
 * key.
 */
@Path("/v1/analytics")
...
public class AnalyticsControllerV1 {
        ...
        @ApiResponse(
            ...
            content =
                @Content(
                    mediaType = MediaType.APPLICATION_JSON,
                    examples =
                        @ExampleObject(value = "{\"error\":\"The input data is invalid.\"}"))),
        @ApiResponse(
            responseCode = "500",
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "User identified and traits persisted to BigQuery successfully"),
        @ApiResponse(
            responseCode = "400",
            description =
                "Invalid request data — payload is missing or both userId and anonymousId are null",
            content =
                @Content(
                    mediaType = MediaType.APPLICATION_JSON,
                    examples =
  ...
  public Response identity(String requestBody) {
    ...
    try {
      ...
      client.upsertUserSnapshot(
          identifyPayload.userId, identifyPayload.anonymousId, identifyPayload.traits);
    } catch (Exception e) {
      return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
          .entity(
              """
