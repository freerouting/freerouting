 ...
 *
 * <p>This filter is the billing-grade source of truth for API traffic: it captures status,
 * latency, caller identity, a hashed API key, and a normalized route template on every tracked
 * path (including early 4xx/5xx aborts that never reach a controller).</p>
 *
 * <p>Excluded paths match telemetry/docs endpoints ({@code /v1/analytics/*}, OpenAPI, Swagger UI,
 * {@code /.well-known/*}, {@code /dev/*}). {@code /v1/system/*} is tracked.</p>
 *
 * <h2>Priority</h2>
...
public class ApiUsageFilter implements ContainerRequestFilter, ContainerResponseFilter {
  ...
  public void filter(ContainerRequestContext requestContext) throws IOException {
    ...
    requestContext.setProperty(PROP_API_ROUTE,
        ApiUsagePaths.normalizeRoute(requestContext.getMethod(), requestContext.getUriInfo().getPath(true)));
    requestContext.setProperty(PROP_API_KEY_HASH, hashBearerToken(requestContext.getHeaderString(AUTHORIZATION_HEADER)));
    requestContext.setProperty(PROP_PROFILE_ID, trimToNull(requestContext.getHeaderString(PROFILE_ID_HEADER)));
    requestContext.setProperty(PROP_PROFILE_EMAIL, trimToNull(requestContext.getHeaderString(PROFILE_EMAIL_HEADER)));
    requestContext.setProperty(PROP_ENVIRONMENT_HOST, trimToNull(requestContext.getHeaderString(EnvironmentHostValidationFilter.HEADER_NAME)));
    requestContext.setProperty(PROP_REQUEST_BYTES, parseContentLength(requestContext.getHeaders().getFirst("Content-Length")));
  }
