...
/**
 * Provides basic functionality of a board with geometric items. It contains functions such as inserting, deleting, modifying and picking items and elementary checking functions. A board may have one
 * or several layers.
 */
public class BasicBoard implements Serializable {
   ...
   * p_observers is used for synchronisation, if the board is generated by a host database, otherwise it is null. If p_test_level != RELEASE_VERSION, some features may be used, which are still in
   * experimental state. Warnings for debugging may be printed depending on the size of p_test_level.
   */
  ...
  /**
   * Inserts a trace into the board, whose geometry is described by a Polyline. p_clearance_class is the index in the clearance_matrix, which describes the required clearance restrictions to other
  ...
  /**
   * Inserts an obstacle into the board , whose geometry is described by a polygonal shape, which may have holes. If p_component_no != 0, the obstacle belongs to a component.
   */
  ...
  /**
   * Inserts an obstacle belonging to a component into the board p_name is to identify the corresponding ObstacleArea in the component package.
   */
  public ObstacleArea insert_obstacle(Area p_area, int p_layer, Vector p_translation, double p_rotation_in_degree, boolean p_side_changed, int p_clearance_class, int p_component_no, String p_name,
  ...
  /**
   * Inserts a via obstacle area into the board , whose geometry is described by a polygonal shape, which may have holes.
   */
  ...
  /**
   * Inserts a via obstacle belonging to a component into the board p_name is to identify the corresponding ObstacleArea in the component package.
   */
  public ViaObstacleArea insert_via_obstacle(Area p_area, int p_layer, Vector p_translation, double p_rotation_in_degree, boolean p_side_changed, int p_clearance_class, int p_component_no,
  ...
  /**
   * Inserts a component obstacle area into the board , whose geometry is described by a polygonal shape, which may have holes.
   */
  ...
  /**
   * Inserts a component obstacle belonging to a component into the board. p_name is to identify the corresponding ObstacleArea in the component package.
   */
  public ComponentObstacleArea insert_component_obstacle(Area p_area, int p_layer, Vector p_translation, double p_rotation_in_degree, boolean p_side_changed, int p_clearance_class, int p_component_no,
      String p_name, FixedState p_fixed_state) {
    if (p_area == null) {
      FRLogger.warn("BasicBoard.insert_component_obstacle: p_area is null");
      return null;
  ...
  /**
   * Inserts a conduction area into the board , whose geometry is described by a polygonal shape, which may have holes. If p_is_obstacle is false, it is possible to route through the conduction area
  ...
  /**
   * Removes the items in p_item_list. Returns false, if some items could not be removed, because they are fixed.
   */
  ...
  /**
   * Combines the connected traces of this net, which have only 1 contact at the connection point. if p_net_no {@literal <} 0 traces of all nets are combined.
   */
  ...
  /**
   * Looks for traces of the input net on the input layer, so that p_location is on the trace polygon, and splits these traces. Returns false, if no trace was split.
   */
  ...
  /**
   * Checks, if an object with shape p_shape and net nos p_net_no_arr and clearance class p_cl_class can be inserted on layer p_layer without clearance violation.
   */
  ...
  /**
   * Checks, if a trace line with shape p_shape and net numbers p_net_no_arr and clearance class p_cl_class can be inserted on layer p_layer without clearance violation. If p_contact_pins != null, all
   * pins not contained in p_contact_pins are regarded as obstacles, even if they are of the own net.
   */
  ...
  /**
   * Draws all items of the board on their visible layers. Called in the overwritten paintComponent method of a class derived from JPanel. The value of p_layer_visibility is expected between 0 and 1
   * for each layer.
   */
  ...
  /**
   * Returns the list of items on the board, whose shape on layer p_layer contains the point at p_location. If p_layer {@literal <} 0, the layer is ignored. If p_item_selection_filter != null, only
  ...
  /**
   * Returns the minimum clearance requested between items of clearance class p_class_1 and p_class_2. p_class_1 and p_class_2 are indices in the clearance matrix.
   */
  ...
  /**
   * Turns an obstacle area into a conduction area with net number p_net_no If it is convex and has no holes, it is turned into a Pin, else into a conduction area.
   */
  ...
  /**
   * Stub function overwritten in class RoutingBoard to maintain the autorouter database if necessary.
   */
  ...
  /**
   * Looks if at the input position ends a trace with the input net number, which has no normal contact at that position. Returns null, if no tail is found.
   */
