 ...
 *
 * <p>The counter starts at 1 and increments monotonically. Once it reaches {@link #c_max_id_no}
 * ({@code Integer.MAX_VALUE / 2} ≈ 1 billion), a single warning is emitted and the counter wraps
 * back to 1 to avoid {@code int} overflow and negative IDs. In practice, wrap-around is reached
 * only after billions of item insertions, which corresponds to many days of continuous routing on a
 * large board. Wrapping avoids the log-flooding that the original code caused by warning on every
 ...
 */
public class ItemIdentificationNumberGenerator
  ...
  public int new_no() {
    if (last_generated_id_no >= c_max_id_no) {
      ...
      wrapAroundCount++;
      FRLogger.warn(
          "IdNoGenerator: ID counter reached "
              + c_max_id_no
