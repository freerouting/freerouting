...
public class BoardComparator {
  ...
  public static ComparisonResult compare(
    ...
    for (app.freerouting.board.model.items.Item item : board1.getItems()) {
      if (item instanceof app.freerouting.board.model.items.Trace trace) {
        traceLength1 += trace.getLength() * scale1;
      }
    }
    double traceLength2 = 0.0;
    for (app.freerouting.board.model.items.Item item : board2.getItems()) {
      if (item instanceof app.freerouting.board.model.items.Trace trace) {
  ...
  private static double getMmFactor(RoutingBoard board) {
    return app.freerouting.board.model.structure.Unit.scale(
            1.0, board.communication.unit, app.freerouting.board.model.structure.Unit.MM)
