...
public class BasicBoard implements Serializable {
   ...
   * matrix. observers is used for synchronisation, if the board is generated by a host database,
   * otherwise it is null. If testLevel != RELEASE_VERSION, some features may be used, which are
  ...
  /**
   * Inserts a trace into the board, whose geometry is described by a Polyline. clearanceClass is
  ...
  public PolylineTrace insertTraceWithoutCleaning(
    ...
    PolylineTrace newTrace =
        new PolylineTrace(
            polyline, layer, halfWidth, netNumbers, clearanceClass, 0, 0, fixedState, this);
  ...
  public void insertTrace(
    ...
    PolylineTrace newTrace =
        insertTraceWithoutCleaning(
            polyline, layer, halfWidth, netNumbers, clearanceClass, fixedState);
    if (newTrace == null) {
      return;
    }
  ...
  /**
   * Inserts a via into the board. attachAllowed indicates, if the via may overlap with smd pins of
   * the same net.
   */
  ...
  /**
   * Inserts a via obstacle belonging to a component into the board name is to identify the
   * corresponding ObstacleArea in the component package.
   */
  public ViaObstacleArea insertViaObstacle(
      Area area,
  ...
  /**
   * Inserts a component obstacle belonging to a component into the board. name is to identify the
   * corresponding ObstacleArea in the component package.
   */
  public ComponentObstacleArea insertComponentObstacle(
      Area area,
  ...
  public ComponentOutline insertComponentOutline(
      ...
      FixedState fixedState) {
    if (area == null) {
      FRLogger.warn("BasicBoard.insert_component_outline: area is null");
      return null;
    }
    if (!area.isBounded()) {
      FRLogger.warn("BasicBoard.insert_component_outline: area is not bounded");
      return null;
    }
   ...
   * which may have holes. If isObstacle is false, it is possible to route through the conduction
  ...
  public ConductionArea insertConductionArea(
      ...
      FixedState fixedState) {
    if (area == null) {
      FRLogger.warn("BasicBoard.insert_conduction_area: area is null");
      return null;
    }
    ConductionArea c =
        new ConductionArea(
  ...
  public BoardOutline insertOutline(PolylineShape[] outlineShapes, int clearanceClassIndex) {
  ...
  /**
   * Removes the items in itemList. Returns false, if some items could not be removed, because they
   * are fixed.
   */
   ...
   * if netNumber {@literal <} 0 traces of all nets are combined.
   */
  public boolean combineTraces(int netNumber) {
    ...
    while (somethingChanged) {
      ...
      for (; ; ) {
        ...
        if ((netNumber < 0 || currentItem.containsNet(netNumber))
  ...
  public boolean normalizeTraces(int netNumber) {
    if (normalizeSuppressedNetNos == null) {
      normalizeSuppressedNetNos = new HashSet<>();
    }
    if (normalizeSuppressedNetNos.contains(netNumber)) {
      String netName =
          rules != null && rules.nets != null && rules.nets.get(netNumber) != null
    ...
    while (somethingChanged) {
      if (++iterationCount > MAX_NORMALIZE_ITERATIONS) {
        ...
        normalizeSuppressedNetNos.add(netNumber);
        break;
      }
      somethingChanged = false;
    ...
    for (; ; ) {
      ...
      if (currentItem instanceof PolylineTrace currentTrace && currentItem.isOnTheBoard()) {
        for (int netNumber : currentTrace.netNumbers) {
          tracesByNet
              .computeIfAbsent(netNumber, k -> new java.util.ArrayList<>())
              .add(currentTrace);
        }
      }
    }
  ...
  /**
   * Looks for traces of the input net on the input layer, so that location is on the trace polygon,
   * and splits these traces. Returns false, if no trace was split.
   */
  public boolean splitTraces(Point location, int layer, int netNumber) {
    ...
    for (Item currentItem : pickedItems) {
      ...
      if (currentTrace.containsNet(netNumber)) {
  ...
  /**
   * Returns all items on layer layer, which overlap with area. If layer {@literal <} 0, the layer
   * is ignored
   */
    ...
    for (int i = 0; i < tiles.length; i++) {
      ...
      for (SearchTreeObject currentObject : obstacles) {
        ...
        for (int j = 0; j < netNumbers.length; j++) {
          if (!currentObject.isObstacle(netNumbers[j])) {
            isObstacle = false;
          }
        }
        if (isObstacle) {
    ...
    for (TreeEntry currentTreeEntry : treeEntries) {
      ...
      if (contactPins != null) {
        ...
        if (currentItem instanceof Pin) {
          // The contact pins of the trace should be contained in ignoreItems.
      ...
      for (int i = 0; i < netNumbers.length; i++) {
        if (!currentItem.isTraceObstacle(netNumbers[i])) {
          isObstacle = false;
        }
      }
  ...
  /**
   * Returns the list of items on the board, whose shape on layer layer contains the point at
   * location. If layer {@literal <} 0, the layer is ignored. If itemSelectionFilter != null, only
  ...
  /** Checks, if point is contained in the bounding box of this board. */
  public boolean contains(Point point) {
    return point.isContainedIn(boundingBox);
  }

  /**
   * Returns the minimum clearance requested between items of clearance class class1 and class2.
   * class1 and class2 are indices in the clearance matrix.
   */
  ...
  /** Returns a box containing all items in itemList. */
  ...
  /** Enlarges the graphics update box, so that it contains box. */
  ...
  /**
   * Turns an obstacle area into a conduction area with net number netNumber If it is convex and has
   * no holes, it is turned into a Pin, else into a conduction area.
   */
  public Connectable makeConductive(ObstacleArea area, int netNumber) {
    Item newItem;
   ...
   * Puts the numbers of the changed nets into the set changedNets, if changedNets != null
   */
   ...
   * the numbers of the changed nets into the set changedNets, if changedNets != null
   */
   ...
   */
  public Trace getTraceTail(Point location, int layer, int[] netNumbers) {
    TileShape pointShape = TileShape.getInstance(location);
    Collection<SearchTreeObject> foundItems = overlappingObjects(pointShape, layer);
    for (SearchTreeObject currentObject : foundItems) {
      if (currentObject instanceof Trace currentTrace) {
        if (!currentTrace.netsEqual(netNumbers)) {
          continue;
        }
  ...
  /**
   * Checks, if item item is part of a cycle and remove it together with its connection in this
   * case.
   */
  public boolean removeIfCycle(Trace trace) {
    ...
    int[] currentNetNumbers = trace.netNumbers;
    ...
    for (int i = 0; i < 2; i++) {
      Trace tail = getTraceTail(endCorners[i], currentLayer, currentNetNumbers);
  ...
  public void areThereItemsOnInactiveLayer(AutorouteControl ctrl) {
    if (this.getLayerCount() > 2) {
      ...
      for (; ; ) {
        UndoableObjects.Storable currentObject = this.itemList.readObject(it);
        if (currentObject == null) {
          break;
        }
        if (currentObject instanceof PolylineTrace currentItem) {
  ...
  public int getNon45DegreeTraceCount() {
    int count = 0;
    for (Item currentObject : getItems()) {
   ...
   * method of a class derived from JPanel. The value of layerVisibility is expected between 0 and 1
   * for each layer.
   */
