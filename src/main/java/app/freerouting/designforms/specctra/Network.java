  ...
  private static boolean read_net_pins(IJFlexScanner p_scanner, Collection<Net.Pin> p_pin_list) {
    ...
    while((component_name = ((SpecctraDsnFileReader)p_scanner).next_string(true, '-')) != "") {
      ...
      try {
        ...
        if(!"-".equals(next_token)) {
          FRLogger.warn("Network.read_net_pins: component name specified without pin name in 'pins' definition, skipping.  Component name: " + component_name);
          continue;
        }
