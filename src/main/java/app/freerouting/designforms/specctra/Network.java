...
public class Network extends ScopeKeyword
{
    ...
    public boolean read_scope(ReadScopeParameter p_par)
    {
        ...
        Collection<app.freerouting.rules.ViaInfo> via_infos = new LinkedList<app.freerouting.rules.ViaInfo>();
        ...
        for (;;)
        {
            ...
            if (prev_token == OPEN_BRACKET)
            {
                ...
                {
                    app.freerouting.rules.ViaInfo curr_via_info = read_via_info(p_par.scanner, p_par.board_handling.get_routing_board());
    ...
    public static void write_via_infos(app.freerouting.rules.BoardRules p_rules, IndentFileWriter p_file, IdentifierType p_identifier_type)
    ...
    public static void write_via_rules(app.freerouting.rules.BoardRules p_rules, IndentFileWriter p_file, IdentifierType p_identifier_type)
            throws java.io.IOException
    {
    ...
    public static void write_net_class(app.freerouting.rules.NetClass p_net_class, WriteScopeParameter p_par)
            throws java.io.IOException
    {
        p_par.file.start_scope();
    ...
    private static void write_circuit(app.freerouting.rules.NetClass p_net_class, WriteScopeParameter p_par)
            throws java.io.IOException
    {
    ...
    static app.freerouting.rules.ViaInfo read_via_info(Scanner p_scanner, app.freerouting.board.BasicBoard p_board)
    {
        try
        {
            ...
            app.freerouting.library.Padstack via_padstack = p_board.library.get_via_padstack(padstack_name);
            ...
            return new app.freerouting.rules.ViaInfo(name, via_padstack, clearance_class, attach_allowed, p_board.rules);
        } catch (java.io.IOException e)
        {
    ...
    private static void insert_via_infos(Collection<app.freerouting.rules.ViaInfo> p_via_infos, RoutingBoard p_board, boolean p_attach_allowed)
    {
    ...
    private static void create_default_via_infos(app.freerouting.board.BasicBoard p_board, app.freerouting.rules.NetClass p_net_class, boolean p_attach_allowed)
    {
        int cl_class = p_net_class.default_item_clearance_classes.get(app.freerouting.rules.DefaultItemClearanceClasses.ItemClass.VIA);
        ...
        for (int i = 0; i < p_board.library.via_padstack_count(); ++i)
        {
            ...
            app.freerouting.rules.ViaInfo found_via_info =
                    new app.freerouting.rules.ViaInfo(via_name, curr_padstack, cl_class, attach_allowed, p_board.rules);
            p_board.rules.via_infos.add(found_via_info);
        }
    }

    private static void insert_via_rules(Collection<Collection<String>> p_via_rules, app.freerouting.board.BasicBoard p_board)
    {
     ...
     */
    static boolean add_via_rule(Collection<String> p_name_list, app.freerouting.board.BasicBoard p_board)
    {
    ...
    static void insert_net_class(NetClass p_class, LayerStructure p_layer_structure, app.freerouting.board.BasicBoard p_board, CoordinateTransform p_coordinate_transform,
    ...
    static private void insert_class_pair_info(NetClass.ClassClass p_class_class, app.freerouting.rules.NetClass p_first_class, app.freerouting.rules.NetClass p_second_class,
                                               app.freerouting.board.BasicBoard p_board, CoordinateTransform p_coordinate_transform)
    {
    ...
    static private void add_mixed_clearance_rule(app.freerouting.rules.ClearanceMatrix p_clearance_matrix, app.freerouting.rules.NetClass p_first_class,
                                                 app.freerouting.rules.NetClass p_second_class, Rule.ClearanceRule p_clearance_rule, int p_layer_no,
    ...
    static private void create_default_clearance_classes(app.freerouting.rules.NetClass p_net_class,
    ...
    private static void create_via_rule(Collection<String> p_use_via, app.freerouting.rules.NetClass p_net_class, app.freerouting.board.BasicBoard p_board, boolean p_attach_allowed)
    {
        app.freerouting.rules.ViaRule new_via_rule = new app.freerouting.rules.ViaRule(p_net_class.get_name());
        int default_via_cl_class = p_net_class.default_item_clearance_classes.get(app.freerouting.rules.DefaultItemClearanceClasses.ItemClass.VIA);
    ...
    private static void add_clearance_rule(app.freerouting.rules.ClearanceMatrix p_clearance_matrix, app.freerouting.rules.NetClass p_net_class,
     ...
     */
    static private int get_clearance_class(app.freerouting.rules.ClearanceMatrix p_clearance_matrix,
                                           app.freerouting.rules.NetClass p_net_class, String p_item_class_name)
    {
        ...
        for (PartLibrary.LogicalPart next_part : p_par.logical_parts)
        {
            ...
            for (PartLibrary.PartPin curr_part_pin : next_part.part_pins)
            {
                ...
                board_part_pins[curr_index] =
                        new app.freerouting.library.LogicalPart.PartPin(pin_no, curr_part_pin.pin_name,
     ...
     */
    private static app.freerouting.library.Package search_lib_package(String p_part_name,
                                                                              java.util.Collection<PartLibrary.LogicalPartMapping> p_logical_part_mappings, app.freerouting.board.BasicBoard p_board)
    {
            ...
            ReadScopeParameter p_par)
    {
        ...
        app.freerouting.board.Component new_component = routing_board.components.add(p_location.name, component_location,
        ...
        for (int i = 0; i < curr_package.pin_count(); ++i)
        {
            ...
            app.freerouting.library.Padstack curr_padstack = routing_board.library.padstacks.get(curr_pin.padstack_no);
            if (curr_padstack == null)
            {
            ...
            if (clearance_class < 0)
            {
                ...
                {
                    clearance_class = net_class.default_item_clearance_classes.get(app.freerouting.rules.DefaultItemClearanceClasses.ItemClass.SMD);
                }
                else
                {
                    clearance_class = net_class.default_item_clearance_classes.get(app.freerouting.rules.DefaultItemClearanceClasses.ItemClass.PIN);
                }
            }
        ...
        for (int k = 0; k <= 2; ++k)
        {
            ...
            for (int i = 0; i < keepout_arr.length; ++i)
            {
                ...
                int clearance_class =
                        routing_board.rules.get_default_net_class().default_item_clearance_classes.get(app.freerouting.rules.DefaultItemClearanceClasses.ItemClass.AREA);
