...
class Structure extends ScopeKeyword
{
    ...
    public boolean read_scope(ReadScopeParameter p_par)
    {
        ...
        for (;;)
        {
            ...
            if (prev_token == OPEN_BRACKET)
            {
                ...
                else if (next_token == Keyword.SNAP_ANGLE)
                {
                    ...
                    app.freerouting.board.AngleRestriction snap_angle = read_snap_angle(p_par.scanner);
                    if (snap_angle != null)
                    {
        ...
        while (it.hasNext())
        {
            ...
            app.freerouting.geometry.planar.Area plane_area =
            ...
            if (curr_layer.no >= 0)
            {
                ...
                {
                    clearance_class_no = curr_net.get_class().default_item_clearance_classes.get(app.freerouting.rules.DefaultItemClearanceClasses.ItemClass.AREA);
                }
    ...
    public static void write_scope(WriteScopeParameter p_par) throws java.io.IOException
    {
        ...
        for (;;)
        {
            ...
            if (curr_ob instanceof app.freerouting.board.BoardOutline)
            {
                break;
            }
        }
        ...
        app.freerouting.board.BoardOutline outline = (app.freerouting.board.BoardOutline) curr_ob;
        ...
        for (;;)
        {
            ...
            app.freerouting.board.ObstacleArea curr_keepout = (app.freerouting.board.ObstacleArea) curr_ob;
        ...
        for (;;)
        {
            ...
            app.freerouting.board.ConductionArea curr_area = (app.freerouting.board.ConductionArea) curr_ob;
    ...
    private static void write_via_padstacks(app.freerouting.library.BoardLibrary p_library, IndentFileWriter p_file,
                                            IdentifierType p_identifier_type) throws java.io.IOException
    {
        ...
        {
            app.freerouting.library.Padstack curr_padstack = p_library.get_via_padstack(i);
    ...
    private static void write_control_scope(app.freerouting.rules.BoardRules p_rules, IndentFileWriter p_file) throws java.io.IOException
    {
        p_file.start_scope();
    ...
    private static void write_keepout_scope(WriteScopeParameter p_par, app.freerouting.board.ObstacleArea p_keepout) throws java.io.IOException
    {
        app.freerouting.geometry.planar.Area keepout_area = p_keepout.get_area();
        ...
        p_par.file.start_scope();
        if (p_keepout instanceof app.freerouting.board.ViaObstacleArea)
        {
    ...
    static app.freerouting.board.AngleRestriction read_snap_angle(IJFlexScanner p_scanner)
    {
        try
        {
            ...
            {
                snap_angle = app.freerouting.board.AngleRestriction.NINETY_DEGREE;
            }
            else if (next_token == Keyword.FORTYFIVE_DEGREE)
            {
                snap_angle = app.freerouting.board.AngleRestriction.FORTYFIVE_DEGREE;
            }
            else if (next_token == Keyword.NONE)
            {
                snap_angle = app.freerouting.board.AngleRestriction.NONE;
            }
            else
            {
    ...
    static void write_snap_angle(IndentFileWriter p_file, app.freerouting.board.AngleRestriction p_angle_restriction) throws java.io.IOException
    {
        p_file.start_scope();
        p_file.write("snap_angle ");
        p_file.new_line();

        if (p_angle_restriction == app.freerouting.board.AngleRestriction.NINETY_DEGREE)
        {
            p_file.write("ninety_degree");
        }
        else if (p_angle_restriction == app.freerouting.board.AngleRestriction.FORTYFIVE_DEGREE)
        {
    ...
    private boolean create_board(ReadScopeParameter p_par, BoardConstructionInfo p_board_construction_info)
    {
        ...
        while (5 * max_coor >= app.freerouting.geometry.planar.Limits.CRIT_INT)
        {
            scale_factor /= 10;
            max_coor /= 10;
        }
        ...
        app.freerouting.rules.ClearanceMatrix clearance_matrix = app.freerouting.rules.ClearanceMatrix.get_default_instance(board_layer_structure, 0);
        app.freerouting.rules.BoardRules board_rules = new app.freerouting.rules.BoardRules(board_layer_structure, clearance_matrix);
        app.freerouting.board.Communication.SpecctraParserInfo specctra_parser_info =
                new app.freerouting.board.Communication.SpecctraParserInfo(p_par.string_quote, p_par.host_cad,
                p_par.host_version, p_par.constants, p_par.write_resolution, p_par.dsn_file_generated_by_host);
        app.freerouting.board.Communication board_communication =
                new app.freerouting.board.Communication(p_par.unit, p_par.resolution, specctra_parser_info,
        ...
        app.freerouting.board.BasicBoard board = p_par.board_handling.get_routing_board();
    ...
    private static void insert_missing_power_planes(Collection<Layer> p_layer_info,
            NetList p_netlist, app.freerouting.board.BasicBoard p_board)
    {
        Collection<app.freerouting.board.ConductionArea> conduction_areas = p_board.get_conduction_areas();
        for (Layer curr_layer : p_layer_info)
        {
            ...
            for (app.freerouting.board.ConductionArea curr_conduction_area : conduction_areas)
            {
     ...
     */
    private static boolean clearance_equals(app.freerouting.rules.ClearanceMatrix p_cl_matrix, int p_layer_1, int p_layer_2)
    {
    ...
    {
        app.freerouting.geometry.planar.Area keepout_area =
    ...
    private static void insert_keepout(app.freerouting.board.BasicBoard p_board, app.freerouting.geometry.planar.Area p_area, int p_layer,
                                       String p_clearance_class_name, KeepoutType p_keepout_type, FixedState p_fixed_state)
    {
        ...
        {
            clearance_class_no =
                    p_board.rules.get_default_net_class().default_item_clearance_classes.get(app.freerouting.rules.DefaultItemClearanceClasses.ItemClass.AREA);
        }
        else
        {
