...
public class SpecctraSesFileWriter {
  ...
  public static boolean write(BasicBoard p_board, OutputStream p_output_stream, String p_design_name) {
    ...
    try {
      // Add special characters commonly found in net names that need quoting
  ...
  private static void write_session_scope(BasicBoard p_board, IdentifierType p_identifier_type, IndentFileWriter p_file,
      String p_session_name, String p_design_name) throws IOException {
   ...
   */
  private static void write_components(BasicBoard p_board, IdentifierType p_identifier_type,
      CoordinateTransform p_coordinate_transform, IndentFileWriter p_file, Package p_package)
      throws IOException {
  ...
  private static void write_component(BasicBoard p_board, IdentifierType p_identifier_type,
      CoordinateTransform p_coordinate_transform, IndentFileWriter p_file,
      app.freerouting.board.Component p_component) throws IOException {
    p_file.new_line();
    p_file.write("(place ");
  ...
  private static void write_was_is(BasicBoard p_board, IdentifierType p_identifier_type, IndentFileWriter p_file)
      throws IOException {
    p_file.start_scope();
    p_file.write("was_is");
    Collection<Pin> board_pins = p_board.get_pins();
    for (Pin curr_pin : board_pins) {
      Pin swapped_with = curr_pin.get_changed_to();
      if (curr_pin.get_changed_to() != curr_pin) {
        ...
        if (curr_cmp != null) {
          ...
          p_file.write("-");
          Package.Pin package_pin = curr_cmp.get_package().get_pin(curr_pin.get_index_in_package());
        ...
        if (swap_cmp != null) {
          ...
          p_file.write("-");
          Package.Pin package_pin = swap_cmp.get_package().get_pin(swapped_with.get_index_in_package());
          p_identifier_type.write(package_pin.name, p_file);
        } else {
          FRLogger.warn("SessionFile.write_was_is: component not found");
        }
  ...
  private static void write_padstack(Padstack p_padstack, BasicBoard p_board, IdentifierType p_identifier_type,
      CoordinateTransform p_coordinate_transform, IndentFileWriter p_file)
      throws IOException {
