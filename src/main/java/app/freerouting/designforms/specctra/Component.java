...
public class Component extends ScopeKeyword
{
    ...
    throws java.io.IOException
    {
        ...
        for (int j = 0; j < 3; ++j)
        {
            ...
            for (int i = 0; i < curr_keepout_arr.length; ++i)
            {
                ...
                app.freerouting.board.ObstacleArea curr_obstacle_area = get_keepout(p_par.board, p_component.no, curr_keepout.name);
    ...
    private static app.freerouting.board.ObstacleArea get_keepout(app.freerouting.board.BasicBoard p_board, int p_component_no, String p_name)
    {
        ...
        for(;;)
        {
            app.freerouting.board.Item curr_item = (app.freerouting.board.Item)p_board.item_list.read_object(it);
            if (curr_item == null)
            {
                break;
            }
            if (curr_item.get_component_no() == p_component_no && curr_item instanceof app.freerouting.board.ObstacleArea)
            {
                app.freerouting.board.ObstacleArea curr_area = (app.freerouting.board.ObstacleArea) curr_item;
