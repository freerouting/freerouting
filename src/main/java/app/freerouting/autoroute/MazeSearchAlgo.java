...
public class MazeSearchAlgo {
  ...
  /**
   * Initializes a new instance of MazeSearchAlgo for searching a connection between p_start_items and p_destination_items. Returns null, if the initialisation failed.
   */
  ...
  /**
   * Does a maze search to find a connection route between the start and the destination items. If the algorithm succeeds, the ExpansionDoor and its section number of the found destination is
   * returned, from where the whole found connection can be backtracked. Otherwise, the return value will be null.
   */
  ...
  /**
   * Expands the next element in the maze expansion list. Returns false, if the expansion list is exhausted or the destination is reached.
   */
          ...
          if (!curr_door_is_small && this.ctrl.max_shove_trace_recursion_depth > 0 && obstacle_item instanceof PolylineTrace) {
            if (!shove_trace_room(p_list_element, obstacle_room)) {
              if (ripup_costs > 0) {
                ...
                MazeListElement new_element = new MazeListElement(p_list_element.door, p_list_element.section_no_of_door, p_list_element.backtrack_door, p_list_element.section_no_of_backtrack_door,
                    p_list_element.expansion_value + ripup_costs, p_list_element.sorting_value + ripup_costs, p_list_element.next_room, p_list_element.shape_entry, true, p_list_element.adjustment,
                    true);
    ...
    for (TargetItemExpansionDoor to_door : p_list_element.next_room.get_target_doors()) {
      ...
      if (!p_next_room_is_thick) {
        ...
        if (!check_points[0].equals(check_points[1])) {
          ...
          boolean check_ok = autoroute_engine.board.check_forced_trace_polyline(check_polyline, ctrl.trace_half_width[curr_layer], curr_layer, curr_net_no_arr, ctrl.trace_clearance_class_no,
  ...
  private boolean expand_to_door(ExpansionDoor p_to_door, MazeListElement p_list_element, int p_add_costs, boolean p_next_room_is_thick, MazeSearchElement.Adjustment p_adjustment) {
    ...
    for (int i = 0; i < line_sections.length; i++) {
      ...
      if (p_next_room_is_thick) {
        ...
        if (p_to_door.dimension == 1 && line_sections.length == 1 && p_to_door.first_room instanceof CompleteFreeSpaceExpansionRoom
   ...
   */
  private boolean expand_to_door_section(ExpandableObject p_door, int p_section_no, FloatLine p_shape_entry, MazeListElement p_from_element, int p_add_costs,
    ...
    double expansion_value =
        p_from_element.expansion_value + p_add_costs + shape_entry_middle.weighted_distance(p_from_element.shape_entry.a.middle_point(p_from_element.shape_entry.b), ctrl.trace_costs[layer].horizontal,
  ...
  /**
   * A drill page is inserted between an expansion room and the drill to expand in order to prevent performance problems with rooms with big shapes containing many drills.
   */
  private void expand_to_drill_page(DrillPage p_drill_page, MazeListElement p_from_element) {
    ...
    double sorting_value =
        expansion_value + nearest_point.weighted_distance(from_element_shape_entry_middle, ctrl.trace_costs[layer].horizontal, ctrl.trace_costs[layer].vertical) + this.destination_distance.calculate(
            nearest_point, layer);
  ...
  private void expand_to_other_layers(MazeListElement p_list_element) {
    ...
    if (curr_drill.room_arr[p_list_element.section_no_of_door] instanceof ObstacleExpansionRoom room) {
      ...
      for (; ; ) {
        ...
        ForcedPadAlgo.CheckDrillResult drill_result = ForcedViaAlgo.check_layer(ctrl.via_radius_arr[curr_layer], ctrl.via_clearance_class, ctrl.attach_smd_allowed, curr_room_shape,
            curr_drill.location, curr_layer, net_no_arr, ctrl.max_shove_trace_recursion_depth, 0, autoroute_engine.board);
    ...
    for (int to_layer = via_lower_bound; to_layer <= via_upper_bound; to_layer++) {
      ...
      MazeListElement new_element = new MazeListElement(curr_drill, curr_room_index, curr_drill, p_list_element.section_no_of_door, expansion_value, sorting_value,
          curr_drill.room_arr[curr_room_index], p_list_element.shape_entry, room_ripped, MazeSearchElement.Adjustment.NONE, false);
      this.maze_expansion_list.add(new_element);
    }
  }
  ...
  /**
   * Checks, if the room can be ripped and returns the rip up costs, which are > 0, if the room is ripped and -1, if no ripup was possible. If the previous room was also ripped and contained the same
   * item or an item of the same connection, the result will be equal to ALREADY_RIPPED_COSTS
   */
  ...
  /**
   * Shoves a trace room and expands the corresponding doors. Return false, if no door was expanded. In this case occupation of the door_section by ripup can be delayed to allow shoving the room fromm
  ...
  /**
   * Checks, if the next room contains a destination pin, where evtl. neckdown is necessary. Return the neck down width in this case, or 0, if no such pin was found,
   */
  ...
  /**
   * Checks, if the next room can be entered if the door of p_list_element is small. If p_ignore_item != null, p_ignore_item and all other items directly connected to p_ignore_item are ignored in the
   * check.
   */
