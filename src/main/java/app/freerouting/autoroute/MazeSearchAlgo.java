...
public class MazeSearchAlgo
{
    ...
    private boolean door_is_small(ExpansionDoor p_door, double p_trace_width)
    {
        if (p_door.dimension == 1 || p_door.first_room instanceof CompleteFreeSpaceExpansionRoom && p_door.second_room instanceof CompleteFreeSpaceExpansionRoom)
        {
            ...
            {
                if (this.autoroute_engine.board.get_test_level().ordinal() >= app.freerouting.board.TestLevel.ALL_DEBUGGING_OUTPUT.ordinal())
                {
    ...
    private void expand_to_drill(ExpansionDrill p_drill, MazeListElement p_from_element, int p_add_costs)
    {
        ...
        if (p_from_element.door instanceof DrillPage && p_from_element.backtrack_door instanceof TargetItemExpansionDoor)
        {
            ...
            {
                FloatPoint nearest_exit_corner = ((app.freerouting.board.Pin) from_item).nearest_trace_exit_corner(p_drill.location.to_float(), trace_half_width, layer);
    ...
    private void expand_to_other_layers(MazeListElement p_list_element)
    {
        ...
        if (curr_drill.room_arr[p_list_element.section_no_of_door] instanceof ObstacleExpansionRoom)
        {
            ...
            app.freerouting.library.Padstack curr_obstacle_padstack = ((app.freerouting.board.Via) curr_obstacle_item).get_padstack();
        ...
        else
        {
            ...
            for (;;)
            {
                ...
                else if (drill_result == app.freerouting.board.ForcedPadAlgo.CheckDrillResult.DRILLABLE_WITH_ATTACH_SMD)
                {
                    if (curr_layer == 0)
                    {
    ...
    private static void reduce_trace_shapes_at_tie_pins(Collection<Item> p_item_list, int p_own_net_no, ShapeSearchTree p_autoroute_tree)
    {
        for (Item curr_item : p_item_list)
        {
            if ((curr_item instanceof app.freerouting.board.Pin) && curr_item.net_count() > 1)
            {
                ...
                {
                    if (!(curr_contact instanceof app.freerouting.board.PolylineTrace) || curr_contact.contains_net(p_own_net_no))
                    {
                        continue;
                    }
                    p_autoroute_tree.reduce_trace_shape_at_tie_pin(curr_tie_pin, (app.freerouting.board.PolylineTrace) curr_contact);
                }
            }
        }
    }
    ...
    private boolean room_shape_is_thick(ObstacleExpansionRoom p_obstacle_room)
    {
        ...
        {
            obstacle_half_width = ((app.freerouting.board.Trace) obstacle_item).get_half_width()
        ...
        {
            TileShape via_shape = ((app.freerouting.board.Via) obstacle_item).get_tree_shape_on_layer(this.search_tree, layer);
    ...
    private int check_ripup(MazeListElement p_list_element, Item p_obstacle_item, boolean p_door_is_small)
    {
        ...
        else if (p_obstacle_item instanceof app.freerouting.board.Via)
        {
            ...
            {
                if (!(curr_contact instanceof app.freerouting.board.Trace) || curr_contact.is_user_fixed())
                {
                    return -1;
                }
                ++contact_count;
    ...
    private static double calc_fanout_via_ripup_cost_factor(app.freerouting.board.Trace p_trace)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            if (curr_trace_contact instanceof app.freerouting.board.Pin && curr_trace_contact.first_layer() == curr_trace_contact.last_layer())
            {
                protect_fanout_via = true;
            }
            else if (curr_trace_contact instanceof PolylineTrace && curr_trace_contact.get_fixed_state() == app.freerouting.board.FixedState.SHOVE_FIXED)
            {
    ...
    private double check_neck_down_at_dest_pin(CompleteExpansionRoom p_room)
    {
        ...
        for (TargetItemExpansionDoor curr_target_door : target_doors)
        {
            ...
            {
                return ((app.freerouting.board.Pin) curr_target_door.item).get_trace_neckdown_halfwidth(p_room.get_layer());
            }
        }
        return 0;
    }
