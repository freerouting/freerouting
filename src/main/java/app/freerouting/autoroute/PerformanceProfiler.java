...
/**
 * Simple performance profiler to measure time spent in different routing operations. Thread-safe
 * for use in multi-threaded routing.
 */
public class PerformanceProfiler {
  ...
  public static void printResults() {
    ...
    if (configInfo != null) {
      ...
      FRLogger.debug(
          String.format(
              "  Via Costs: %d, Plane Via Costs: %d",
              configInfo.viaCosts, configInfo.planeViaCosts));
      FRLogger.debug("  Layer Costs (Preferred / Against):");
      for (int i = 0; i < configInfo.preferredCosts.length; i++) {
        FRLogger.debug(
            String.format(
                "    Layer %d: %.1f / %.1f",
                i + 1, configInfo.preferredCosts[i], configInfo.againstCosts[i]));
      }
      FRLogger.debug("");
    }
    ...
    timings.entrySet().stream()
        ...
        .forEach(
            entry -> {
              ...
              FRLogger.debug(
                  String.format(
                      "  %-40s: %8d ms total, %8d calls, %6d ms avg",
    ...
    if (!passHistory.isEmpty()) {
      ...
      synchronized (passHistory) {
        ...
        for (PassInfo pass : passHistory) {
          FRLogger.debug(
              String.format(
                  "  Pass %-3d: %4d unrouted items, %8.2f s, %5d ripup cost",
