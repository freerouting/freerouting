    ...
    public boolean autoroute_passes()
    {
        ...
        while (still_unrouted_items && !this.is_interrupted)
        {
            ...
            if (diffBetweenBoards.size() > diffBetweenBoardsCheckSize) {
                ...
                {
                    FRLogger.warn("There were only " + FRLogger.DefaultFloatFormat.format( average.getAsDouble()) + " changes in the last " + diffBetweenBoardsCheckSize + " passes, so it's very likely that autorouter can't improve the result much further. It is recommended to stop it and finish the board manually.");
