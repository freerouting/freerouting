 ...
 * 1000 represents a fully-routed board with no vias and no traces (the theoretical maximum). Scores
...
public final class BoardScoreBreakdown {
  ...
  /**
   * Penalty contribution from unrouted connections. Equals {@code incompleteCount ×
   * unroutedNetPenalty}.
   */
  public final float unroutedConnectionsPenalty;

  /**
   * Penalty contribution from DRC clearance violations. Equals {@code violationCount ×
   * clearanceViolationPenalty}.
   */
  public final float clearanceViolationsPenalty;

  /**
   * Penalty contribution from trace bends (direction-change corners). Equals {@code bendCount ×
   * bendPenalty}.
   */
  public final float bendsPenalty;

  /**
   * Absolute cost contribution from trace length. Equals {@code totalLengthMm ×
   * defaultPreferredDirectionTraceCost}.
   */
  public final float traceLengthCost;

  /** Absolute cost contribution from via count. Equals {@code viaCount × viaCosts}. */
  ...
  /** Raw (un-normalised) score. {@code rawScore = maximumScore - totalPenalties - totalCosts} */
  public final float rawScore;

  /**
   * Score mapped to the {@code [0, 1000]} range. {@code normalizedScore = max(0, rawScore /
   * maximumScore) × 1000}
   */
  ...
  private BoardScoreBreakdown(
    ...
    this.traceLengthCost =
        (float) (totalTraceLengthMm * weights.defaultPreferredDirectionTraceCost);
    ...
    this.normalizedScore = maximumScore > 0 ? Math.max(0f, rawScore / maximumScore) * 1000f : 0f;
  }
  ...
  private static void validateWeights(ScoringSettings w) {
    ...
    if (w.defaultPreferredDirectionTraceCost == null) {
      throw new IllegalArgumentException(
          "weights.defaultPreferredDirectionTraceCost must not be null");
    }
    if (w.viaCosts == null) {
