 ...
 * pipeline can distinguish "this source has no opinion" (null) from "this source explicitly sets a
 * value". Hard-coded defaults must live exclusively in {@link
 * app.freerouting.settings.sources.DefaultSettings}.
 *
 * <p>Naming conventions:
 *
 * <ul>
 *   <li><b>penalty</b> — subtracted when a quality constraint is violated (unrouted net, DRC
 *       violation, bend). These appear in the board-score formula.
 *   <li><b>costs</b> — subtracted as an absolute cost proportional to resource usage (via count,
 *       trace length). These also appear in the board-score formula.
 *   <li><b>startRipupCosts</b> — a routing-control parameter passed to the maze-search engine; it
 *       does <em>not</em> appear in the board-score formula.
 * </ul>
 */
public class ScoringSettings implements Serializable, Cloneable {
  ...
  @SerializedName(value = "via_costs", alternate = {"viaCosts"})
  ...
  /**
   * Base ripup cost for the first ripup-and-reroute pass. This is a routing-control parameter
   * multiplied by the pass number inside {@code BatchAutorouter}; it does NOT appear in the
   * board-score formula.
   */
  @SerializedName(value = "start_ripup_costs", alternate = {"startRipupCosts"})
  ...
  /**
   * Creates a deep copy of this RouterScoringSettings object. All fields including arrays are
   * cloned.
   *
