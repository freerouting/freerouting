...
/**
 * Loads router settings from the freerouting.json file in the user data folder. Only the settings
 * present in the JSON file will be non-null.
 */
public class JsonFileSettings implements SettingsSource {
  ...
  private RouterSettings loadSettings() {
    ...
    try (Reader reader = Files.newBufferedReader(jsonFilePath)) {
      ...
      FRLogger.warn(
          "Failed to load settings from JSON file: " + jsonFilePath + ": " + e.getMessage());
    } catch (Exception e) {
