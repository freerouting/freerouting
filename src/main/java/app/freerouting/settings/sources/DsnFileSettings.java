...
/**
 * Extracts router settings from DSN (Specctra Design) files. Only the settings present in the DSN
 * file will be non-null.
 */
public class DsnFileSettings implements SettingsSource {
  ...
  /**
   * Creates a DsnFileSettings source from a DSN file input stream. Performs a lightweight
   * metadata-only parse that stops after the {@code (structure ...)} scope — significantly faster
   * than a full board load.
   *
  ...
  public DsnFileSettings(InputStream inputStream, String filename) {
    ...
    this.settings = rs;
    FRLogger.debug(
        "Loaded router settings from DSN file: " + filename + " (" + layerCount + " layers)");
  }
