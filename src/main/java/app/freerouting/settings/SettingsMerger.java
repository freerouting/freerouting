...
/**
 * Central class responsible for merging router settings from multiple sources into a single,
 * resolved {@link RouterSettings} instance.
 *
 * <h2>How merging works</h2>
 *
 * Sources are sorted by ascending priority and applied one on top of another. {@link
 * app.freerouting.settings.sources.DefaultSettings} (priority 0) always provides the initial base.
 * Every subsequent source calls {@link RouterSettings#applyNewValuesFrom}, which delegates to
 * {@link app.freerouting.util.ReflectionUtil#copyFields}. That method iterates the public,
 * non-static fields of the incoming {@code RouterSettings} object and copies a field into the
 * target <em>only when the source value is non-null and not equal to the field's Java default
 * value</em>.
 *
 * <h2>Why all {@code RouterSettings} fields must be nullable (no default initializers)</h2>
 *
 * The null-check inside {@code copyFields} is the sole mechanism that distinguishes "this source
 ...
 * a higher-priority source (e.g. the API) whenever the user left that setting unspecified. Keeping
 * all fields as {@code null} by default ensures that only fields explicitly populated by a source
 * can win at merge time.
 *
 * <h2>Priority order</h2>
 *
 * <ol>
 *   <li>Default settings (priority 0) — lowest priority, applied first as the base
 *   <li>JSON file settings (priority 10) — {@code freerouting.json} in user-data folder
 *   <li>DSN file settings (priority 20) — metadata extracted from the Specctra design file
 *   <li>SES file settings (priority 30) — metadata from Specctra session files
 ...
 *       mode. The concrete {@code InteractiveSettings} singleton is registered here by {@code
 *       GuiBoardManager} immediately after each board load so that every subsequent {@link
 *       #merge()} call reflects the live user-controlled state.
 ...
 *
 * Implement {@link SettingsSource}, choose a priority that fits the desired override order, and
...
public class SettingsMerger implements Cloneable {
   ...
   *       InteractiveSettings} instance (a {@code GuiSettings} subclass) to replace a plain {@code
   *       GuiSettings} placeholder that was registered at startup before the board was loaded.
   * </ol>
   *
