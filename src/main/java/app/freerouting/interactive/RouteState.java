...
public class RouteState extends InteractiveState {
  ...
  public static RouteState get_instance(
      ...
      if (picked_corner instanceof IntPoint
        ...
        if (picked_trace instanceof PolylineTrace) {
          FloatPoint nearest_point =
              ((PolylineTrace) picked_trace).polyline().nearest_point_approx(p_location);
    ...
    boolean is_stitch_route =
        p_board_handling.settings.is_stitch_route
    ...
    new_instance.route =
        new Route(
            ...
            p_board_handling.settings.push_enabled,
            p_board_handling.settings.trace_pull_tight_region_width,
            p_board_handling.settings.trace_pull_tight_accuracy,
            picked_item,
            new_instance.routing_target_set,
            routing_board,
            is_stitch_route,
            p_board_handling.settings.automatic_neckdown,
            p_board_handling.settings.via_snap_to_smd_center,
  ...
  protected static Item start_ok(IntPoint p_location, BoardHandling p_hdlg) {
     ...
     */
    Item picked_item =
        routing_board.pick_nearest_routing_item(p_location, p_hdlg.settings.layer, null);
  ...
  private static Item pick_routing_item(IntPoint p_location, int p_layer_no, BoardHandling p_hdlg) {
    ...
    if (p_layer_no == p_hdlg.settings.layer
  ...
  public InteractiveState cancel() {
    Trace tail =
        hdlg.get_routing_board()
            .get_trace_tail(route.get_last_corner(), hdlg.settings.layer, route.net_no_arr);
    if (tail != null) {
