...
public class SelectItemsInRegionState extends SelectRegionState {
  ...
  private SelectItemsInRegionState(
  ...
  public static SelectItemsInRegionState get_instance(
      InteractiveState p_parent_state,
      BoardHandling p_board_handling,
      ActivityReplayFile p_activityReplayFile) {
  ...
  private void select_all_in_region() {
    ...
    Set<Item> found_items =
        hdlg.settings.item_selection_filter.filter(
    ...
    if (something_found) {
      if (this.return_state instanceof SelectedItemState) {
        ...
        this.return_state =
            SelectedItemState.get_instance(
