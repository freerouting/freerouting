...
public class SelectedItemState extends InteractiveState
{
  ...
  public InteractiveState autoroute(Stoppable p_stoppable_thread)
  {
    ...
    {
      String start_message = tm.getText("autoroute") + " " + tm.getText("stop_message");
    ...
    for (Item curr_item : autoroute_item_list)
    {
      ...
      Net route_net = hdlg.get_routing_board().rules.nets.get(curr_item.get_net_no(0));
      if (route_net != null)
      {
    ...
    if (p_stoppable_thread != null)
    {
      ...
      String end_message = tm.getText("autoroute") + " " + curr_message + ": " + found_count + " " + tm.getText("connections_found") + ", " + not_found_count + " " + tm.getText("connections_not_found");
  ...
  public InteractiveState fanout(Stoppable p_stoppable_thread)
  {
    ...
    if (p_stoppable_thread != null)
    {
      String start_message = tm.getText("fanout") + " " + tm.getText("stop_message");
      hdlg.screen_messages.set_status_message(start_message);
    }
    int not_found_count = 0;
    int found_count = 0;
