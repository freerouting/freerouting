...
public class Route
{
    ...
    private boolean snap_to_smd_center(int p_layer)
    {
        ...
        {
            if (curr_item instanceof app.freerouting.board.Pin && curr_item.shares_net_no(this.net_no_arr))
            {
    ...
    private Set<SwapPinInfo> calculate_swap_pin_infos()
    {
        ...
        for (Item curr_item : this.target_set)
        {
            ...
            {
                Collection<app.freerouting.board.Pin> curr_swapppable_pins = ((app.freerouting.board.Pin) curr_item).get_swappable_pins();
