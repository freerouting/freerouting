...
public class Route
{
  ...
  public boolean next_corner(FloatPoint p_corner)
  {
    ...
    Point ok_point = board.insert_forced_trace_segment(prev_corner, curr_corner, pen_half_width_arr[layer], layer, net_no_arr, clearance_class, max_shove_trace_recursion_depth, max_shove_via_recursion_depth, max_spring_over_recursion_depth, trace_tidy_width, pull_tight_accuracy, !is_stitch_mode, check_forced_trace_time_limit);
  ...
  public boolean change_layer(int p_to_layer)
  {
    ...
    for (int i = 0; i < this.via_rule.via_count(); ++i)
    {
      ...
      result = board.forced_via(curr_via_info, this.prev_corner, this.net_no_arr, clearance_class, pen_half_width_arr, max_shove_trace_recursion_depth, 0, this.trace_tidy_width, this.pull_tight_accuracy, pull_tight_time_limit);
      if (result)
      {
        via_found = true;
        break;
  ...
  private boolean connect(Point p_from_point, IntPoint p_to_point)
  {
    ...
    for (int i = 1; i < corners.length; ++i)
    {
      ...
      {
        Point curr_ok_point = board.insert_forced_trace_segment(from_corner, to_corner, pen_half_width_arr[layer], this.layer, net_no_arr, clearance_class, max_shove_trace_recursion_depth, max_shove_via_recursion_depth, max_spring_over_recursion_depth, trace_tidy_width, pull_tight_accuracy, !is_stitch_mode, time_limit);
  ...
  private Point try_neckdown_at_start(IntPoint p_to_corner)
  {
    ...
    TimeLimit time_limit = new TimeLimit(CHECK_FORCED_TRACE_TIME_LIMIT);
    Point ok_point = board.insert_forced_trace_segment(prev_corner, p_to_corner, neck_down_halfwidth, layer, net_no_arr, clearance_class, max_shove_trace_recursion_depth, max_shove_via_recursion_depth, max_spring_over_recursion_depth, trace_tidy_width, pull_tight_accuracy, !is_stitch_mode, time_limit);
    return ok_point;
  }
  ...
  private Point try_neckdown_at_end(Point p_from_corner, Point p_to_corner)
  {
    ...
    TimeLimit time_limit = new TimeLimit(CHECK_FORCED_TRACE_TIME_LIMIT);
    Point ok_point = board.insert_forced_trace_segment(p_from_corner, p_to_corner, neck_down_halfwidth, layer, net_no_arr, clearance_class, max_shove_trace_recursion_depth, max_shove_via_recursion_depth, max_spring_over_recursion_depth, trace_tidy_width, pull_tight_accuracy, !is_stitch_mode, time_limit);
    return ok_point;
  }
