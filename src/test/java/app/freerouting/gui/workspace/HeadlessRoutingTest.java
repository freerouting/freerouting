...
/**
 * Sub-Issue 07 – Guard headless / API code paths against {@code workspaceSettings} usage.
 *
 * <p>Verifies that a full autoroute pass can complete in headless mode without encountering any
 * {@link NullPointerException} or {@link IllegalStateException} caused by {@code workspaceSettings}
 * being accessed. The Phase-3 contract split makes GUI state statically unreachable in headless
 * mode: {@link app.freerouting.management.HeadlessBoardManager} does not implement {@link
 * WorkspaceContract}, so {@code WorkspaceSettings} can never leak into the headless code path.
 */
