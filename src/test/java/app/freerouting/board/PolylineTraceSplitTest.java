...
/**
 * Test cases for PolylineTrace split operations, particularly around overlap handling and cycle
 * detection.
 */
public class PolylineTraceSplitTest {
  ...
  /**
   * Test that reproduces the bug where a valid trace segment is incorrectly removed during
   * split/overlap operations.
   *
   * <p>This test simulates the scenario from the bug report where: 1. Two traces are combined
   * (trace 360 + 361 -> combined trace 361) 2. A new overlapping trace is inserted (trace 362) 3.
   * The split operation incorrectly removes the combined trace 361 4. Important routing segments
   * are lost
   */
  @Test
  void testSplitDoesNotRemoveValidSegments() {
    ...
    Assertions.assertTrue(
        hasP1 && hasP5,
        "Combined trace should span from "
            + p1
            + " to "
            + p5
            + ", but goes from "
    ...
    try {
      ...
      Assertions.assertTrue(
          hasConnectionFromP1,
          "After split, there should still be a trace connected to the starting point "
              + p1
              + ". The segment from "
              + p1
              + " to "
  ...
  void testSplitPreservesNonOverlappingSegments() {
    ...
    Assertions.assertTrue(
        hasFirstSegment, "First segment (containing p1) should be preserved after split");
    Assertions.assertTrue(
        hasLastSegment, "Last segment (containing p4) should be preserved after split");
  }

  /**
   * Test cycle detection doesn't trigger false positives when both ends of a split piece
   * temporarily touch the same newly-inserted trace.
   */
  @Test
