...
| **DSN** | Default / recommended | Legacy Specctra export/import; all KiCad versions; stable |

| **JSON/API** | Experimental | Opt-in on KiCad 9+ — SWIG board walk → JSON → localhost REST API |



JSON/API mode is **not** KiCad's official protobuf IPC API. It does **not** yet import all KiCad design rules (including copper-to-edge clearance — see [Issue #558](../../docs/issues/Issue558-copper-to-edge-clearance.md)). Set `routing_mode` to `ROUTING_MODE_JSON` in `config.py` to try it; DSN is the default.



Details: [`docs/issues/kicad-json-api-mode-improvement-tracker.md`](../../docs/issues/kicad-json-api-mode-improvement-tracker.md)
