param(
    ...
    [string]$OutputDir = "build/sdk/csharp-client",
    [string]$PackageName = "Freerouting.CSharp.Client",
...
$dockerArgs = @(
    ...
    "--skip-validate-spec",
    "--additional-properties", "packageName=$PackageName,packageVersion=$PackageVersion,targetFramework=$TargetFramework,optionalEmitDefaultValues=true"
)
...
Write-Host "Generating C# SDK into: $outputAbsolute"
...
if ($LASTEXITCODE -ne 0) {
    throw "C# SDK generation failed with exit code $LASTEXITCODE."
}

Write-Host "C# SDK generation completed."