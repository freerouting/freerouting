param(
    ...
    [string]$OutputDir = "build/sdk/python-client",
...
$dockerArgs = @(
    ...
    "--skip-validate-spec",
    "--additional-properties", "packageName=freerouting_python_client,projectName=freerouting-python-client,packageVersion=$PackageVersion,generateSourceCodeOnly=false"
)
...
Write-Host "Generating Python SDK into: $outputAbsolute"
...
if ($LASTEXITCODE -ne 0) {
    throw "Python SDK generation failed with exit code $LASTEXITCODE."
}

Write-Host "Python SDK generation completed."