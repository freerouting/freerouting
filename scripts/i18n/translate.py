...
def validate_placeholders(english: str, translation: str) -> bool:
    english_placeholders = Counter(PLACEHOLDER_RE.findall(english))
    translation_placeholders = Counter(PLACEHOLDER_RE.findall(translation))
    if english_placeholders != translation_placeholders:
        ...
        if missing:
            err(f"      {symbol('warn')} Missing placeholders in translation: {dict(missing)}")
        if extra:
            err(f"      {symbol('warn')} Extra placeholders in translation: {dict(extra)}")
        return False
    if english.count("\\n") != translation.count("\\n"):
        err(f"      {symbol('warn')} Literal line breaks were not preserved in translation")
        return False
    return True
...
def translate_items_batch(
    ...
    if parsed is not None and len(parsed) == len(keys):
        for key, english_value, entry, _previous in batch:
            ...
            full_entry = enrich_entry(entry if entry else default_entry(bundle, key, english_value), english_value)
            if not validate_placeholders(english_value, translation):
                failures += 1
                failed_keys.add(key)
                continue
