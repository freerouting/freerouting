...
- **`InteractiveSettings` / GUI Session State:** `InteractiveSettings` extends `GuiSettingsSource` (which implements `SettingsSource` at priority 65) and is the **sole** live source of GUI state in the `SettingsMerger` pipeline. Key invariants:
  ...
  - `InteractiveSettings.getSettings()` returns a **live snapshot** of the current GUI state and is what `SettingsMerger` reads at priority 65. Do not cache the result; always call `merger.merge()` to get an up-to-date `RouterSettings`.
...
- **`SettingsMerger.addOrReplaceSources(SettingsSource)`:** When registering a `GuiSettingsSource`-subtype source (e.g., the `InteractiveSettings` singleton), pass the concrete instance — the merger uses subtype matching to find and replace any existing `GuiSettingsSource` entry at priority 65. Never register a plain `GuiSettingsSource` instance after the singleton has been registered or it will silently shadow the live GUI state.
...
The full priority ladder is documented in `docs/settings.md`. Key sources: `DefaultSettings` (0), `JsonFileSettings` (10), `DsnFileSettings` (20), `SesFileSettings` (30), `RulesFileSettings` (40), `EnvironmentVariablesSource` (55), `CliSettings` (60), `GuiSettingsSource` / `InteractiveSettings` (65), `ApiSettings` (70).
