...
public class InsertFoundConnectionAlgo {
  ...
  private Point try_neck_down(
    ...
    double curr_clearance = this.board.rules.clearance_matrix.get_value(
    ...
    double ok_length = board.check_trace_segment(
        p_from_corner,
        p_to_corner,
        p_layer,
        net_no_arr,
    ...
    if (ok_length <= TOLERANCE) {
      ...
      IntPoint add_corner = LocateFoundConnectionAlgo.calculate_additional_corner(
          float_from_corner,
      ...
      curr_ok_point = board.insert_forced_trace_segment(
          add_corner,
          neck_down_end_point,
          ctrl.trace_half_width[p_layer],
          p_layer,
      ...
      add_corner = LocateFoundConnectionAlgo.calculate_additional_corner(
          float_neck_down_end_point,
          float_to_corner,
          !horizontal_first,
          board.rules.get_trace_angle_restriction())
   ...
   * that a forced via is
   * possible at p_location with this mask and inserts the via. Returns false, if
   * no suitable via
