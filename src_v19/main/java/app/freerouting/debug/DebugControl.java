...
public class DebugControl {
    ...
    // This is just a default placeholder for forward-compatibility with the current version
    public static final DebugSettings debugSettings = new DebugSettings();
     ...
     */
    public void reset() {
        if (debugSettings.singleStepExecution) {
            pause();
        } else {
            resume();
        }
    ...
    public boolean isInterested(String impactedItems) {
        ...
        return debugSettings.isNetPermitted(netNo, null);
    }
    ...
    public boolean check(String operation, String impactedItems) {
        ...
        if (!debugSettings.singleStepExecution &&
                debugSettings.traceInsertionDelay == 0) {
            return false;
        }

        int netNo = getNetNo(impactedItems);

        if (!debugSettings.isNetPermitted(netNo, null)) {
            return false;
        }
    ...
    public boolean check(String operation, int netNo, String netName) {
        ...
        if (netNo >= 0 && !debugSettings.isNetPermitted(netNo, netName)) {
            return false;
        }
        ...
        // Handle Delay
        if (debugSettings.traceInsertionDelay > 0) {
            try {
                Thread.sleep(debugSettings.traceInsertionDelay);
    ...
    private boolean isInterestedInOperation(String operation) {
        ...
        for (String filterOp : debugSettings.operationFilters) {
